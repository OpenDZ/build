...
rm -f efi-disk.img
dd if=/dev/null of=efi-disk.img bs=1M seek=512 count=1
parted --script efi-disk.img "mklabel gpt" "mkpart ESP fat32 1MiB 511MiB" "set 1 boot on"
...
cp ../boot-efi/bootx64.efi $ROOT/EFI/Boot/bootx64.efi
...
objcopy \
  ...
  ../boot-efi/stubx64.efi $ROOT/EFI/bus1/bus1.efi
